        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex flex-col items-center">
                    ...
                    <!-- Form Upload Foto -->
                    <form action="{{ route('peserta.profile.update-foto') }}" method="POST" enctype="multipart/form-data" id="fotoProfilForm">
                        @csrf
                    ...
                    @if($user->foto_profil)
                    <form action="{{ route('peserta.profile.delete-foto') }}" method="POST" class="mt-2">
                        @csrf
                        @method('DELETE')
                    ...
                    <p class="text-gray-600 text-sm mt-1">Peserta Magang</p>
                    <div class="mt-4 w-full">
                        <div class="bg-blue-50 rounded-lg p-3 text-center">
                            <p class="text-xs text-gray-600">Status</p>
                            <p class="text-sm font-semibold text-blue-600">
                                @if($peserta && $peserta->periode_end && $peserta->periode_end->isFuture())
                                    Aktif
                                @else
                                    Selesai
                                @endif
            ...
            <div class="bg-white rounded-lg shadow-md p-6 mt-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-center text-sm">
                        ...
                        <span class="text-gray-600">Mulai: {{ $peserta ? $peserta->periode_start->format('d F Y') : '-' }}</span>
                    </div>
                    <div class="flex items-center text-sm">
                        <svg class="w-5 h-5 text-gray-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                        </svg>
                        <span class="text-gray-600">Fungsi: {{ $peserta->fungsi ?? '-' }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div id="dataDiriView" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="text-sm font-medium text-gray-600">NIM (Opsional)</label>
                            <p class="mt-1 text-gray-800">{{ $peserta->nim ?? '-' }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-600">Institut/Universitas</label>
                            <p class="mt-1 text-gray-800">{{ $peserta->institut ?? '-' }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-600">Fungsi</label>
                            <p class="mt-1 text-gray-800">{{ $peserta->fungsi ?? '-' }}</p>
                        </div>
                        <div>
                            ...
                            <p class="mt-1 text-gray-800">
                                @if($peserta && $peserta->periode_start && $peserta->periode_end)
                                    {{ $peserta->periode_start->format('d F Y') }} - {{ $peserta->periode_end->format('d F Y') }}
                                @else
                                    -
                                @endif
                            </p>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-800">{{ $peserta->email ?? '-' }}</p>
                        </div>
                    </div>
                </div>

                <!-- Edit Mode -->
                <form id="dataDiriEdit" action="{{ route('peserta.profile.update-data-diri') }}" method="POST" class="hidden space-y-4">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="nim" value="{{ $peserta->nim ?? '' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Institut/Universitas</label>
                            <input type="text" name="institut" value="{{ $peserta->institut ?? '' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                        </div>
                        <div>
                            ...
                            <select name="fungsi" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                                <option value="Technical" {{ ($peserta->fungsi ?? '') == 'Technical' ? 'selected' : '' }}>Technical</option>
                                <option value="Marketing" {{ ($peserta->fungsi ?? '') == 'Marketing' ? 'selected' : '' }}>Marketing</option>
                                <option value="Finance" {{ ($peserta->fungsi ?? '') == 'Finance' ? 'selected' : '' }}>Finance</option>
                                <option value="Human Resources" {{ ($peserta->fungsi ?? '') == 'Human Resources' ? 'selected' : '' }}>Human Resources</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Mulai</label>
                            <input type="date" name="periode_start" value="{{ $peserta && $peserta->periode_start ? $peserta->periode_start->format('Y-m-d') : '' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Selesai</label>
                            <input type="date" name="periode_end" value="{{ $peserta && $peserta->periode_end ? $peserta->periode_end->format('Y-m-d') : '' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Email</label>
                            <input type="email" name="email" value="{{ $peserta->email ?? '' }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="mb-6 pb-6 border-b border-gray-200">
                    ...
                    <form id="usernameEdit" action="{{ route('peserta.profile.update-username') }}" method="POST" class="hidden space-y-4">
                        @csrf
                        <div>
                ...
                <div>
                    ...
                    <form id="passwordEdit" action="{{ route('peserta.profile.update-password') }}" method="POST" class="hidden space-y-4">
                        @csrf
                        <div>
