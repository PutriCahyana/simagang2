    ...
    <div class="container-fluid">
        ...
        <div class="mentor-header">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1><i class="fas fa-user-shield mr-2"></i>Dashboard Admin</h1>
                    <p>Selamat datang, <strong>{{ $admin->nama }}</strong></p>
                </div>
                <div class="col-md-4 text-right">
                    <div class="stats-card d-inline-block">
                        <div class="stats-label">Total Peserta Aktif</div>
        ...
        <!-- Room Cards -->
        <h5 class="section-title">Semua Room</h5>
        <div class="row mb-4">
                    ...
                    <div class="card-body">
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                ...
                                <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $room['jumlah_peserta'] }} Peserta Aktif</div>
                                <div class="text-xs text-muted mt-2">
                                    <i class="fas fa-chalkboard-teacher mr-1"></i>Mentor: {{ $room['mentor_nama'] }}
                                </div>
                            </div>
                            <div class="col-auto">
        ...
        <p class="text-muted small mb-3">Menampilkan periode yang sedang berjalan dari semua peserta. Klik untuk melihat detail</p>
        <div class="row mb-4">
            @foreach($periodeData as $periode)
            @php
                // Hitung progress untuk ring
...
function showAllPesertaDetail() {
    $.get('/admin/all-peserta-detail', function(data) {
        ...
        if (data.length === 0) {
            ...
            data.forEach((item, index) => {
                tbody.append(`
                    <tr>
                        <td>${index + 1}</td>
                        <td><a href="/admin/home/peserta/${item.id}" class="text-primary font-weight-bold">${item.nama}</a></td>
                        <td>${item.institut}</td>
                        <td>${item.periode}</td>
...
function showRoomDetailById(roomId) {
    ...
    $.get(`/admin/room/${roomId}/detail`, function(response) {
...
function showPeriodDetail(periode, days) {
    ...
    $.get(`/admin/period/${encodeURIComponent(periode)}`, function(data) {
        const tbody = $('#periodTableBody');
        tbody.empty();
        
        if (data.length === 0) {
            tbody.append('<tr><td colspan="6" class="text-center">Tidak ada peserta di periode ini</td></tr>');
        } else {
            data.forEach((item, index) => {
                tbody.append(`
                    <tr>
                        ...
                        <td><a href="/admin/home/peserta/${item.id}" class="text-primary font-weight-bold">${item.nama}</a></td>
                        <td>${item.institut}</td>
                        <td><span class="badge badge-secondary">${item.room}</span></td>
...
function showInstitutDetail(institut, type) {
    ...
    $.get(`/admin/institut/${encodeURIComponent(institut)}/${type}`, function(data) {
        const tbody = $('#institutTableBody');
        tbody.empty();
        
        if (data.length === 0) {
            tbody.append('<tr><td colspan="6" class="text-center">Tidak ada peserta dari institut ini</td></tr>');
        } else {
            data.forEach((item, index) => {
